...
def lambda_handler(event, context, delete_message=None):
    ...
    #put
        ...
        put_message(input)
    #delete
    elif input['path'] == messages_id_path and input['http_method'] == 'DELETE':
...
def create_message(body: dict):
    ...
    msg = {'msg':f'Message created: {json.dumps(body)}'}
    logger.info(f'Successfully posted new message to db: {params}')
    return build_response(http.HTTPStatus.CREATED, msg)
